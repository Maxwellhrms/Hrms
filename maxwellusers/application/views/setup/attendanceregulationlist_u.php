                                ...
                                <div class="card-body">	
									<div class="table-responsive">
                                        <table class="datatable table table-stripped mb-0" id="dataTables-example1">
                                            ...
                                            <tbody>
                                                <?php   //echo '<pre>';   print_r($authresult);exit;                                         
                                                // foreach($authresult['userdata'][1] as $val){
                                                    if(sizeof($attnd) > 0 ){
                                                    foreach($attnd as $authv){
                                                    ?>
                                            <tr>
...
$("form#addendregulmodel").submit(function (e) {
    // alert('dvbhjv')
    e.preventDefault();
    var mainurl = baseurl+'approveauthreguattendance';
...
$(document).ready(function () {
    $('.attndreg').on('click', function () {
        var data = $(this).data('id'); // Get full data-id
        var parts = data.split('~');   // Split into array

        var uniqid = parts[0]; // Get the uniqid (first value)

        $('#unid').val(uniqid); // Set hidden input value
    });
});
