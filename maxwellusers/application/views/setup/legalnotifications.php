...
<div class="content container-fluid">
									...
									<div class="row">
										<div class="col-md-3"> 
											<div class="form-group form-focus select-focus">
												...
												<select class="  " style="width: 100%" name="category" id="category" tabindex="-1" aria-hidden="true" data-select2-id="category"> 
				                                    <option value="" data-select2-id="22"> Category </option>
				                                    <option value="" data-select2-id="51">Select</option>
													<option value="CERTIFICATESRENEWALS" data-select2-id="52">CERTIFICATE RENEWALS </option>
													<option value="courcases" data-select2-id="53">court cases</option>
													<option value="General" data-select2-id="54">General issues</option>
													<option value="HRSTATUTORY" data-select2-id="55">HR STATUTORY PAYMENTS </option>
													<option value="nclt" data-select2-id="56">NCLT cases </option>
													<option value="police" data-select2-id="57">police complaints </option>
													<option value="RENTALAGREEMENTS" data-select2-id="58">RENTAL AGREEMENTS </option>												</select>
  ...
  <div class="form-group">
    ...
    <div class="input-group date" id="hearingdatePicker" data-target-input="nearest">
      <input type="text" name="hearingdate" id="hearingdate" class="form-control datetimepicker-input" data-target="#hearingdatePicker"/>
    ...
    <span class="formerror" id="hearingdate_error"></span>
										...
										<div class="col-md-3"> 
											<div class="form-group form-focus select-focus">
												<select class="" style="width: 100%" name="ym" id="ym"> 
                        ...
                        <table class="datatable table table-stripped mb-0" id="custom_dataTables">
							...
							<tbody>
								...
								<tr>
									...
									<?php //$reminder = config('notification_reminder'); 
									      $reminder[0] = ''; 
									if($val->mx_ntf_followup_date == ''){$rm = $val->mx_ntf_hearing_date;}else{ $rm = $val->mx_ntf_followup_date; } ?>
						...
						<div class="modal-body">
							<form id="addnewfilestodocument" method="POST" enctype='multipart/form-data'>
								...
								<div class="col-sm-2">
									<div class="form-group">
										<label>&nbsp;</label>
										<button type="button" id="add_neweditfile" class="form-cont4rol btn btn-info add_lang_btn"><i class="fa fa-plus" style="color:#fff;"></i></button>
									</div>
								</div>
								</section>
								</div>
...
$("form#addlegalform").submit(function(e) {
	...
	mainurl = baseurl+'savelegalnotifications';
...
function getdetails(app, id){
	event.preventDefault();
	    $.ajax({
	        url: baseurl+'getlegalnotifications',
	        type: 'POST',
...
function filter(){
    ...
    var esi_state_id = $("#esi_state_id").val();
    var esi_branch_id = $("#esi_branch_id").val();
    window.location.href = "<?php echo base_url() ?>legalnotification?category=" + category + "&esi_company_id=" + esi_company_id +
  ...
  $(".modal-body #editticketid").val(id);
   ...
   $('#add_neweditfile').click(function(e) {
    ...
    $("form#addnewfilestodocument").submit(function(e) {
         e.preventDefault(); 
         var formData = new FormData(this);
         mainurl = baseurl+'addnewfilestodocument';
         $.ajax({
            url: mainurl,
            type: 'POST',
            data: formData,
...
<!-- Bootstrap 4 (for styling and datetimepicker) -->
